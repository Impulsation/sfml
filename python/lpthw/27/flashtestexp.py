...
def printCardIndex(x, index):
    raw_input("%s: %s" % (index[x].printNumber(), index[x].printFront()))
    raw_input("%s: %s" % (index[x].printNumber(), index[x].printBack()))
    print ""

def runCards():
    for i in range(0, len(card_index)):
        printCardIndex(i, card_index)
